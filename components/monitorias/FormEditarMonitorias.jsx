...
import { updateMonitoria } from "utils/monitorias";

const FormEditarMonitorias = () => {
  const { monitores, setMonitorias, editar, setEditar } =
    useContext(InitialContext);
  const [error, setError] = useState(false);
  const { materia, monitor, fecha, salon } = editar;
  ...
  const onHandleSubmit = (e) => {
    ...
    //enviar data
    updateMonitoria(editar, setMonitorias);
    setEditar(false);
  };

  return (
    <>
      <h1>Editar monitorias</h1>
...
export default FormEditarMonitorias;
